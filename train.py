...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    dataloader, dataset = create_dataloader(train_path, imgsz, batch_size, gs, opt,
                                            hyp=hyp, augment=True, cache=opt.cache_images, rect=opt.rect, rank=rank,      # augment=False
...
if __name__ == '__main__':
    ...
    parser.add_argument('--project', default='runs/train', help='save to project/name') ########################################### default='drive/MyDrive/colab/CV_jiejing/yolov5/runs/train'
    parser.add_argument('--name', default='exp', help='save to project/name')
    parser.add_argument('--exist-ok', action='store_true', help='existing project/name ok, do not increment')
    opt = parser.parse_args()

    # Set DDP variables 设置DDP模式的参数
    opt.total_batch_size = opt.batch_size
    opt.world_size = int(os.environ['WORLD_SIZE']) if 'WORLD_SIZE' in os.environ else 1 # world_size:表示全局进程个数
    opt.global_rank = int(os.environ['RANK']) if 'RANK' in os.environ else -1 # global_rank:进程编号
    ...
    if opt.resume:  # resume an interrupted run
        ckpt = opt.resume if isinstance(opt.resume, str) else get_latest_run()  # specified or most recent path 如果resume是str,则表示传入的是模型的路径地址
                                                                                                              # get_latest_run()函数获取runs文件夹中最近的last.pt
        assert os.path.isfile(ckpt), 'ERROR: --resume checkpoint does not exist'
        with open(Path(ckpt).parent.parent / 'opt.yaml') as f: # opt参数也全部替换
            opt = argparse.Namespace(**yaml.load(f, Loader=yaml.FullLoader))  # replace
        opt.cfg, opt.weights, opt.resume = '', ckpt, True
        logger.info('Resuming training from %s' % ckpt) # ckpt='./drive/MyDrive/colab/CV_jiejing/yolov5/runs/train/exp2/weights/last.pt'
    else:
    ...
    # Evolve hyperparameters (optional)
        ...
        meta = {'lr0': (1, 1e-5, 1e-1),  # initial learning rate (SGD=1E-2, Adam=1E-3)
                ...
                'anchors': (2, 2.0, 10.0),  # anchors per output grid (0 to ignore) ########################################### 加#
        ...
        for _ in range(300):  # generations to evolve ########################################### evolve次数
            ...
            print_mutation(hyp.copy(), results, yaml_file, opt.bucket) ########################################### 修改utils.general.print_mutation
